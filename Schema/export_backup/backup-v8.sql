...
CREATE TABLE `milestone_settings` (
  `step_setting_id` int NOT NULL AUTO_INCREMENT,
  `semester_id` varchar(20) NOT NULL COMMENT 'e.g., "2025/2026-1"',
  `step_sequence` int NOT NULL COMMENT '1, 2, 3, 4, 5 etc.',
  `phase_title` varchar(100) NOT NULL COMMENT 'e.g., "Chapter 1 Submission"',
  `required_artifacts` json NOT NULL COMMENT 'JSON array tracking expected data tags: ["CH1_pdf", "github_url"]',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`step_setting_id`),
  UNIQUE KEY `idx_semester_sequence` (`semester_id`,`step_sequence`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `projects_submissions` (
  ...
  `submission_type` varchar(50) NOT NULL COMMENT 'Decoupled from static ENUMs for fluid coordinator splitting support',
  `status` enum('pending','approved','rejected') NOT NULL DEFAULT (_utf8mb4'pending'),
  `project_id` int NOT NULL,
  `step_setting_id` int DEFAULT NULL,
  `artifact_key` varchar(50) DEFAULT NULL COMMENT 'Matches specific tokens in required_artifacts array',
  PRIMARY KEY (`submission_id`),
  KEY `project_id` (`project_id`),
  KEY `fk_submissions_milestone_settings` (`step_setting_id`),
  CONSTRAINT `fk_submissions_milestone_settings` FOREIGN KEY (`step_setting_id`) REFERENCES `milestone_settings` (`step_setting_id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
CREATE TABLE `salutations` (
  ...
  UNIQUE KEY `idx_unique_title` (`title_name`)
...
CREATE TABLE `users` (
  ...
  `salutation_id` int NOT NULL DEFAULT '1',
