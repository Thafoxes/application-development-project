...
<template>
  <div v-if="isOpen" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100]">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 flex flex-col gap-4 relative">
      ...
      <div class="flex flex-col gap-1">
        ...
        <input v-model="form.title" type="text" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-[#5c001f] focus:border-[#5c001f] outline-none" />
      </div>
      ...
      <div class="flex justify-end gap-3 mt-4">
        <button @click="close" class="px-4 py-2 text-gray-600 font-medium hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
        <button @click="save" class="px-4 py-2 bg-[#5c001f] text-white font-medium rounded-lg hover:bg-[#4a0019] transition-colors">Save Changes</button>
      </div>
    </div>
  </div>
</template>
