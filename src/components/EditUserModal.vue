...
watch(() => props.user, (newUser) => {
  if (newUser) {
    ...
    if (newUser.expertise) {
      expertiseTags.value = newUser.expertise.split(',').map(t => t.trim()).filter(Boolean)
    } else {
      expertiseTags.value = []
    }
  }
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div class="bg-white border border-gray-300 rounded-lg p-8 shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-[#5c001f]">Edit User</h2>
        <button @click="emit('close')" class="text-gray-500 hover:text-red-600 font-bold text-xl">&times;</button>
      </div>
      ...
      <form @submit.prevent="submitForm" class="flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-2">
          ...
          <input v-model="form.fullName" type="text" required class="border border-gray-300 p-3 rounded focus:outline-none focus:border-[#5c001f]" />
        </div>

        <!-- Email -->
        <div class="flex flex-col gap-2">
          <label class="font-semibold text-gray-800">Email Address *</label>
          <input v-model="form.email" type="email" required class="border border-gray-300 p-3 rounded focus:outline-none focus:border-[#5c001f]" />
        </div>

        <!-- Phone Number -->
        <div class="flex flex-col gap-2">
          <label class="font-semibold text-gray-800">Phone Number</label>
          <input v-model="form.phoneNumber" type="tel" maxlength="12" class="border border-gray-300 p-3 rounded focus:outline-none focus:border-[#5c001f]" />
        </div>

        <!-- Affiliation -->
        <div class="flex flex-col gap-2">
          <label class="font-semibold text-gray-800">Affiliation</label>
          <input v-model="form.affiliation" type="text" class="border border-gray-300 p-3 rounded focus:outline-none focus:border-[#5c001f]" />
        </div>
        ...
        <div class="mt-6 flex justify-end gap-4">
          <button type="button" @click="emit('close')" class="px-6 py-3 font-semibold text-gray-600 hover:text-gray-900 transition-colors">
            Cancel
          </button>
