    ...
    >
          ...
          >
            ...
            >
              <div v-if="msg.images && msg.images.length > 0" class="w-full flex justify-end">
                <img :src="'data:image/jpeg;base64,' + msg.images[0]" class="max-w-[200px] rounded-lg border border-white/20" />
              </div>
              <div v-html="renderMarkdown(msg.content)"></div>
            </div>
            
            <!-- Agent Confirmation Card -->
            <div v-if="msg.action === 'CONFIRM_CREATE_USER' && !msg.completed" class="w-full mt-3 bg-white p-4 rounded-lg border border-gray-200 text-black shadow-sm flex flex-col gap-3">
              <h4 class="font-bold text-[#5C001F] m-0">Confirm New User Details</h4>
              <div class="flex flex-col gap-2 text-sm">
                <label class="flex flex-col text-gray-600 font-medium">Name: <input v-model="msg.payload.fullName" class="border px-2 py-1.5 w-full rounded focus:outline-none focus:border-[#5C001F] font-normal text-black"></label>
                <label class="flex flex-col text-gray-600 font-medium">Email: <input v-model="msg.payload.email" class="border px-2 py-1.5 w-full rounded focus:outline-none focus:border-[#5C001F] font-normal text-black"></label>
                <label class="flex flex-col text-gray-600 font-medium">Password: <input v-model="msg.payload.password" class="border px-2 py-1.5 w-full rounded focus:outline-none focus:border-[#5C001F] font-normal text-black"></label>
                <label class="flex flex-col text-gray-600 font-medium">Phone number: <input v-model="msg.payload.phoneNumber" class="border px-2 py-1.5 w-full rounded focus:outline-none focus:border-[#5C001F] font-normal text-black"></label>
                <label class="flex flex-col text-gray-600 font-medium">Affiliation: <input v-model="msg.payload.affiliation" class="border px-2 py-1.5 w-full rounded focus:outline-none focus:border-[#5C001F] font-normal text-black"></label>
                <label class="flex flex-col text-gray-600 font-medium">Organization: <input v-model="msg.payload.coOrgName" class="border px-2 py-1.5 w-full rounded focus:outline-none focus:border-[#5C001F] font-normal text-black"></label>
                <label class="flex flex-col text-gray-600 font-medium">Expertise: <input v-model="msg.payload.expertise" class="border px-2 py-1.5 w-full rounded focus:outline-none focus:border-[#5C001F] font-normal text-black"></label>
                
                <div class="flex justify-end gap-2 mt-2">
                  <button @click="cancelAction(msg)" class="px-3 py-1.5 bg-gray-200 hover:bg-gray-300 rounded text-gray-700 font-semibold border-none cursor-pointer transition-colors">Cancel</button>
                  <button @click="executeCreateUser(msg)" class="px-3 py-1.5 bg-[#5C001F] hover:bg-[#4a0019] text-white rounded font-bold border-none cursor-pointer transition-colors flex items-center gap-1">
                    <span v-if="msg.isExecuting" class="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
                    Confirm & Create
                  </button>
                </div>
              </div>
      ...
      <div class="p-3 bg-white border-t rounded-b-xl shrink-0 m-0 relative">
        ...
        <div v-if="selectedImagePreview" class="mb-2 relative inline-block">
          <img :src="selectedImagePreview" class="h-16 w-16 object-cover rounded-lg border border-gray-200 shadow-sm" />
          <button @click="clearImage" type="button" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 h-5 w-5 flex items-center justify-center border-none shadow-sm cursor-pointer hover:bg-red-600">
            <X class="h-3 w-3" />
          </button>
        </div>
...
const chatHistory = ref([
  { role: 'assistant', content: 'Hello! I am the I-FAMOUS AI Assistant. How can I help you today?' }
])
...
const executeCreateUser = async (msg) => {
  ...
  try {
    const token = localStorage.getItem('token')
    const response = await axios.post('http://localhost:3000/api/assistant/execute-user-creation', msg.payload, {
      headers: { Authorization: `Bearer ${token}` }
    })
    
    if (response.data.success) {
      msg.completed = true;
      chatHistory.value.push({ role: 'assistant', content: "Success! The user was created successfully in the database." })
    } else {
      chatHistory.value.push({ role: 'assistant', content: `Error: ${response.data.error || 'Failed to create user.'}` })
    }
  } catch (error) {
    chatHistory.value.push({ role: 'assistant', content: `Error: ${error.response?.data?.error || 'Could not connect to server.'}` })
  } finally {
