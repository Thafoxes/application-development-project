<template>
  <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden w-full max-w-6xl mx-auto">
    <!-- Header Area -->
    <div class="bg-[#5C001F] text-white px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="flex items-center bg-white/10 rounded-lg p-1">
          <button 
            ...
            :class="[
              ...
              viewMode === 'month' ? 'bg-white text-[#5C001F] shadow-sm' : 'text-white/80 hover:text-white hover:bg-white/10'
            ]"
          >
            Month
          </button>
          <button 
            ...
            :class="[
              'px-4 py-1.5 rounded-md text-sm font-medium transition-colors',
              viewMode === 'week' ? 'bg-white text-[#5C001F] shadow-sm' : 'text-white/80 hover:text-white hover:bg-white/10'
            ]"
          >
            Week
          </button>
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center bg-white/10 rounded-lg overflow-hidden">
          <button @click="previous" class="p-2 hover:bg-white/20 transition-colors" title="Previous">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
          </button>
          <button @click="goToToday" class="px-3 py-1.5 text-sm font-medium hover:bg-white/20 transition-colors border-x border-white/10">
            Today
          </button>
    ...
    <div v-if="viewMode === 'month'" class="p-4 md:p-6 bg-gray-50/50">
      <div class="grid grid-cols-7 gap-px bg-gray-200 rounded-lg overflow-hidden border border-gray-200">
        <!-- Day headers -->
        <div v-for="day in daysOfWeek" :key="day" class="bg-gray-100 py-3 text-center text-sm font-semibold text-gray-700">
          {{ day }}
        </div>
    ...
    <div v-else class="flex flex-col bg-white">
      <div class="flex border-b border-gray-200 bg-gray-50">
        ...
        <div class="flex-1 grid grid-cols-7">
          ...
          >
            <span class="text-xs text-gray-500 uppercase font-semibold">{{ daysOfWeekShort[index] }}</span>
            <span 
...
const getEventsForDate = (dateStr) => {
  return props.events.filter(e => e.date === dateStr).sort((a, b) => {
