    ...
    <div class="flex flex-1 w-full relative">
      ...
      <main class="flex-1 flex flex-col px-[50px] py-[30px] overflow-y-auto">
        ...
        <div class="flex flex-col lg:flex-row gap-8 mt-2 h-full">
          ...
          <div class="w-[300px] flex flex-col bg-[#FFFFAB] border border-gray-200 rounded-xl overflow-hidden shrink-0 relative shadow-sm h-[800px]">
            <div class="p-6 pb-2">
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              ...
              <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <svg class="w-4 h-4 text-gray-600 transition-transform duration-200" :class="{ 'rotate-180': accordions.lecturers }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                
                <div v-show="accordions.lecturers" class="border-t border-gray-100 p-3 space-y-3">
                  <div v-if="calendarStore.availableSchedules.lecturers.length === 0" class="text-xs text-gray-400">
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2">
                      <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden shrink-0">
                        <svg class="w-5 h-5 text-gray-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" /></svg>
                      </div>
                      <div class="flex flex-col min-w-0">
                        <span class="text-xs font-medium text-gray-900 truncate" :title="schedule.label">{{ schedule.label }}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <svg class="w-4 h-4 text-gray-600 transition-transform duration-200" :class="{ 'rotate-180': accordions.classes }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                
                <div v-show="accordions.classes" class="border-t border-gray-100 p-3 space-y-3">
                  <div v-if="calendarStore.availableSchedules.classes.length === 0" class="text-xs text-gray-400">
                    No class sections
                  </div>
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2 text-gray-700 min-w-0">
                      ...
                      <span class="text-sm font-medium truncate" :title="schedule.label">{{ schedule.label }}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="flex-1 flex flex-col gap-4">
            ...
            <div v-if="selectedScheduleId" class="flex items-end gap-4 p-4 bg-white/60 border border-gray-300 rounded-lg shadow-sm">
              <div class="flex flex-col gap-1 flex-1">
                <label class="text-sm font-medium text-gray-700">Owner Identifier (Staff Email or Section)</label>
                <input 
            ...
            <!-- Calendar Container -->
            <div v-if="selectedScheduleId" class="bg-[#e7ded3] rounded-lg overflow-hidden shadow-lg border border-gray-300 relative">
                ...
                <div v-for="(week, wIdx) in calendarWeeks" :key="wIdx" class="grid grid-cols-7 border-b border-gray-300 last:border-b-0">
                  <div v-for="(cell, dIdx) in week" :key="dIdx" class="min-h-[120px] border-r border-gray-300 last:border-r-0 p-1 flex flex-col gap-1 relative" :class="!cell.day ? 'bg-gray-200/50' : 'bg-white hover:bg-gray-50 transition-colors'">
                    ...
                    <template v-if="cell.day">
                      <div v-for="slot in cell.recurringSlots" :key="slot.slot_id" @click="editRecurringSlot(slot.dayOfWeek, slot.slot_id)" class="bg-[#e85d04] text-white rounded p-1.5 text-[10px] leading-tight flex flex-col cursor-pointer hover:opacity-90 relative group shadow-sm">
            ...
            <!-- Empty State -->
            <div v-else class="flex-1 bg-white/50 rounded-xl border border-gray-300 flex items-center justify-center shadow-inner min-h-[500px]">
              <p class="text-xl text-gray-500 font-bold">Please select a time table from the left menu to edit.</p>
            </div>
